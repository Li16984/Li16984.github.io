# 李鹏博 生物信息学笔记 

## 学习计划

#### 学习计划时间表

<table>
  <tr>
    <th rowspan=2 width='120px' align='center'>
      时间
    </th>
    <th rowspan=2 width='140px' align='center'>
      学习阶段
    </th>
    <th colspan=2 align='center'>
      学习任务
    </th>
  </tr>
  <tr>
    <th align='center' width='240px'>
      编程基础
    </th>
    <th align='center' width='240px'>
      生物信息学分析
    </th>
  </tr>
  <tr>
    <td align='center'>
      Week 1-4
    </td>
    <td align='center'>
      基础知识
    </td>
    <td>
      学习Linux操作系统和Linux Bash命令语言，补充基础编程知识
    </td>
    <td>
      复习生物学基础知识（基因组学、测序技术等）<br>学习基本分析方法（序列比对、系统发生分析、功能富集分析等）
    </td>
  </tr>
  <tr>
    <td align='center'>
      Week 5-8
    </td>
    <td align='center'>
      生信基本工具
    </td>
    <td>
      学习R语言，尝试使用R语言进行数据处理和统计分析
    </td>
    <td>
      学习从数据库下载测序数据<br>学习二代测序数据分析流程（测序数据向参考基因组的映射，转录组分析等）
    </td>
  </tr>
  <tr>
    <td align='center'>
      Week 9-12
    </td>
    <td align='center'>
      生信分析实践
    </td>
    <td>
      尝试使用R语言完成小型生信分析项目
    </td>
    <td>
      学习更多测序数据分析（RNA-seq、ChIP-seq等）和生物大分子相互作用分析<br>尝试独立完成小型项目（突变位点分析、基因表达热图绘制等）
    </td>
  </tr>
  <tr>
    <td align='center'>
      Week 13-16
    </td>
    <td align='center'>
      内容整合和拓展学习
    </td>
    <td>
      初步了解和实践基于Python或R语言的机器学习
    </td>
    <td>
      尝试实现完整项目的分析（获取原始序列数据、差异基因筛选、分析特征，得出结论）并撰写报告
    </td>
  </tr>
</table>

#### 学习材料

##### 1 教材

  1. 101计划《生物信息学》
  2. 《鸟哥的Linux私房菜－基础学习篇》
  3. 《Bioinformatics Data Skills》

##### 2 线上材料

  1. [Bioinformatics Tutorial](https://book.ncrnalab.org/teaching/)
  2. [Teaching Videos](https://cloud.tsinghua.edu.cn/d/82e9fca41d634ce6b5df/?p=%2F&mode=list)

#### 3 学习计划个人想法

  1. 逐渐熟悉Markdown语言，尝试在课上直接使用Markdown语言记笔记
  2. 每周课后花费**4-5小时**完成**课后作业**，以及**Tutorial**内容的复习和练习
  3. 每周将代码备份至Github
  4. 本人有一定Python基础，故从第二周开始计划每周**至少2小时**自主学习并练习**Linux bash**和**R语言**

## Week 01 Introduction and Getting Start

#### 课程目标：

> 我们将教授生物信息学方面的专业技能，这些技能不是单纯地运行软件的能力，而是让你们拥有**探索各种真实数据的自由**。

#### 课程内容

* Programming Skills
* Basic & NGS Data Analyses
* Machine Learning & AI

### 1. Introduction

#### Question/Hypothesis-driven Science


* Question → Information → Analysis → Modeling

* 始于对于现象的一个具体的**科学问题**或**假设**，核心目标为**验证**假设或解释现象的**因果关系**

#### Big Data-driven Science

* Information → Analysis → Modeling → Question

* 始于一组现成的**大规模数据集**，核心目标为**发现**数据的未知组织模式或找到数据间的**相关性**

**Data-driven范式的关键背景技术：** 大数据获取（NGS，二代测序技术）和数据分析与挖掘技术（机器学习算法）

#### 算法（Algorithm）与模型（Model）的区别

（参考与Deepseek交流结果）

<table>
  <tr>
    <th width='60px'>
      &nbsp 
    </th>
    <th>
      算法（Algorithm）
    </th>
    <th>
      模型（Model）
    </th>
  </tr>
  <tr>
    <th>
      本质
    </th>
    <td>
      一组特定的<b>计算步骤</b>，定义了解决某一类问题的数学逻辑
    </td>
    <td>
      一个具体的数学结构（如数学表达式、参数集合等），包含了从特定数据集中学到的具体知识
    </td>
  </tr>
  <tr>
    <th>
      功能
    </th>
    <td>
      对特定的问题输入给出<b>解答</b>，或通过处理数据、<b>产生一个模型</b>
    </td>
    <td>
      基于已有的数据集对新数据进行<b>判断</b>或<b>预测</b>
    </td>
  </tr>
  <tr>
    <th>
      例子
    </th>
    <td>
      动态规划算法<br>最小二乘回归算法
    </td>
    <td>
      线性回归模型<br>决策树模型
    </td>
  </tr>
</table>

### 2. Getting Start

#### Linux操作系统

一种基于命令行的开源多用户操作系统

* Docker：轻量化的虚拟机，用于运行Linux环境
* Linux Bash：一种通用的Linux**命令解析器**，也是一种擅长处理字符串的**脚本编程语言**

#### Bioinformatics Learning Tools

* Markdown：简洁的通用**标记语言**，格式为.md，用于编写网页，学习中可以用于编写笔记
* Github：代码托管平台，构建仓库以存储个人笔记与代码
* Vim：Linux环境常用的编辑器，不依赖鼠标，可完全通过键盘操作编辑
* VS Code：常用的编辑器，现支持AI插件
